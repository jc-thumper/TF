...
class SummarizeDataLine(models.Model):
    ...
    def update_summarize_data(self, created_date=None, **kwargs):
        try:
            ...
            line_ids = forecast_level_obj.update_records_for_summarize_data_line(obj=self,
                                                                                 model=self.env['summarize.data.line'],
                                                                                 created_date=created_date)

            # Step: update demand to serversumm_lines
            _logger.info("Processing %s records in Summarize Data Line: %s", len(line_ids), line_ids)
            pack_size = 100
    ...
    def get_summarize_data_vals_to_update_fral(self, summarized_record):
        """
    ...
    @job(retry_pattern={1: 1 * 60,
                        ...
                        9: 30 * 60},
         default_channel='root.forecasting')
    def update_adjust_line_table(self, summ_line_ids, **kwargs):
        ...
        :param summ_line_ids: list of id of records in table `summarize_data_line`
        :type summ_line_ids: list
        :return: None
        """
        try:
            _logger.info('Update forecast result adjust line table')
            new_summ_lines = self.search([('id', 'in', summ_line_ids)])
            fore_adj_line = self.env['forecast.result.adjust.line']

            dict_data = list(new_summ_lines.mapped(lambda line:
                                                   self.get_summarize_data_vals_to_update_fral(line)))
            if dict_data:
                company_id = dict_data[0].get('company_id')
                if company_id:
                    adj_line_constrain_cols = fore_adj_line._list_constrain_columns(company_id)
                    fore_adj_line.create_mul_rows(dict_data,
