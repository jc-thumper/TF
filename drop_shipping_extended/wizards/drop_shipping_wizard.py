...
class DropShippingWizard(models.TransientModel):
    ...
    def default_get(self,vals):
        ...
        stock_obj=self.env['stock.picking'].search([('id','in',active_ids)])
        ...
        if stock_obj.state!='done' and stock_obj.state !='cancel':
            ...
            for obj in stock_obj.move_lines:
                if obj.state != 'cancel' and obj.state != 'done':
                    ...
                    terms.append((0, 0, {'product_id':obj.product_id.id,'ref_id':obj.id,'product_qty':obj.product_qty,'product_uom':obj.product_uom.id,'unit_price':obj.product_id.standard_price,'vendor_id':vendor_id,'drop_boolean' : False,'procurement_id':obj.sale_line_id.id}))
    ...
    def action_create_drop_ship(self):
        ...
        for obj in self.line_ids:
            if obj.vendor_id:
                ...
                raise UserError(_('No Vendor Configured For Product  %s') %(obj.product_id.name))
